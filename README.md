...
## 本来想写一个轻量级的节省token，但是发现效果很差，主要是不多加点规范或者提高工程量，比如说上agent loop或者加一个agent约束，ai就容易胡言乱语，但是加了这些又跟轻量级初衷相悖，还不如丢个skill给openclaw或者cc了，总之这是个废弃的半成品

> 与 AI 对话，知识自动沉淀到本地 Markdown 知识库，支持 Git 版本管理。

## 核心功能

- **智能对话**：接入 OpenAI 兼容 API（DeepSeek / OpenAI / Ollama），流式实时回复
